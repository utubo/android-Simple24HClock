...
def dms_to_decimal(dms_str):
    match = re.match(r'([+-])(\d{2})(\d{2})(\d{2})?([+-])(\d{3})(\d{2})(\d{2})?', dms_str)
    if not match: return 0.0, 0.0
    
    def to_dec(sign, d, m, s):
        res = int(d) + int(m)/60.0 + (int(s or 0)/3600.0)
        return res if sign == '+' else -res

    lat = to_dec(match.group(1), match.group(2), match.group(3), match.group(4))
    lng = to_dec(match.group(5), match.group(6), match.group(7), match.group(8))
...
for line in raw_data.strip().split('\n'):
    ...
    lat, lng = dms_to_decimal(parts[1])
    entry = json.dumps({"id": parts[2], "lat": lat, "lng": lng}, separators=(',', ':'))
